...
export default async function AdminBundleInsertSetsPage() {
  ...
  const [{ data: sets }, { data: brands }, { data: products }] = await Promise.all([
    supabase
  ...
  return (
    <div>
      ...
      <BundleInsertSetsList
        ...
        brands={(brands ?? []) as { id: string; name: string; code: string | null }[]}
        products={(products ?? []) as { id: string; name: string; smaregi_product_id: string | null }[]}
      />
    </div>
  );
}
