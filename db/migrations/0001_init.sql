...
create table if not exists users (
  ...
  -- Firebase Authentication(Identity Platform)のUID文字列。Cloud SQL側にはSupabaseの
  -- auth.usersに相当するテーブルが存在しないため、外部キーではなく単なる一意テキストとして保持する。
...
-- Cloud SQL移行時の注記: Supabase版ではここで全テーブルにRLSを有効化していたが、
-- ポリシーは一つも定義しておらず(サーバーAPIの単一DBロール経由のみでアクセスする方針)、
-- Cloud SQL側でもアプリ用の単一権限ロールのみが接続するため、RLSの有効化自体を行わない。
